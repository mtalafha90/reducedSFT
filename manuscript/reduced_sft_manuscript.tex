...
\citep{Babcock1961,Leighton1964,Yeates2023}. The amplitude of the
...
amplitude, first demonstrated independently by \citet{Lemerle2017} and
\citet{KarakMiesch2017} to stabilise Babcock--Leighton dynamos as
indicated by observations, with direct magnetogram evidence for its form
provided by \citet{Jha2020} \citep[see also][]{DasiEspuig2010,Jiao2021};
latitude quenching, the poleward shift of the activity belts in stronger
cycles combined with the strong latitude dependence of the dipole
contribution of emerging flux \citep{Jiang2020,Talafha2022}, a shift that
arises naturally from the nonlinearity of magnetic buoyancy in the rise of
the stronger toroidal flux \citep{Biswas2022}; and converging surface
inflows towards the activity belts, which reduce the cross-equatorial
separation of polarities
\citep{CameronSchussler2012,MartinBelda2017,Teweldebirhan2024,Talafha2025}.
...
amplitude. Direct magnetogram measurements of bipolar-region tilts show
quenching of this kind \citep{Jha2020}, and Babcock--Leighton dynamo
models demonstrate that it suffices to stabilise the cycle
\citep{Lemerle2017,KarakMiesch2017}.
...
of the drifting activity belt. The physical origin of the poleward
displacement of emergence in stronger cycles is the nonlinearity of
magnetic buoyancy in the rise of the toroidal flux: stronger fields become
buoyant earlier in the cycle, when the toroidal band lies at higher
latitudes \citep{Biswas2022}. The effective poleward displacement
...
multiplicative reduction $1-b_{\rm IQ}T^2+\mathcal{O}(T^4)$; and the
...
range, $b_{\rm IQ}=f/(1-f)$ with $f\simeq0.17$, gives
%
\begin{equation}
b_{\rm IQ}=0.2,
\label{eq:bI_value}
...
belts with an amplitude scaling as $T^2$
\citep[for the nonlinear impact of observed inflows in three-dimensional
Babcock--Leighton dynamo models, see][]{Teweldebirhan2024}. This mode is
used for the physical validation in
Section~\ref{sec:physical_validation}; the reference results use the
...
Q_{\rm IQ}(T), & \mathrm{inflow\text{-}like};
\end{cases}
...
$Q_{\rm LQ}$ and $Q_{\rm IQ}$ on the amplitude), with interaction terms of
...
Beyond the boundary~(\ref{eq:period_doubling}) the iterated map undergoes
a period-doubling cascade into chaotic modulation, with the cascade onset
at the analytic boundary (Figure~\ref{fig:app_bifurcation}).
...
-
\frac{2b_{\rm IQ}T}{1+b_{\rm IQ}T^2}
-
\frac{
2b_{\rm LQ}T
\left[
...
$\mathcal{M}'(T_*)=-1$ overlaid (Figure~\ref{fig:stability_map}), the
saturated amplitude $T_*$ (Figure~\ref{fig:tstar_map}), and the
multiplier $\mathcal{M}'(T_*)$ (Figure~\ref{fig:mprime_map}). The structure follows the analytic results
...
at $\bar{t}_\omega=6.5$~yr, two years \emph{after} cycle maximum
(Figure~\ref{fig:yield_weights}), because early-cycle flux emerges at high
latitudes where its yield is small.
...
\begin{figure}
\centering
\includegraphics[width=\linewidth]{figF3_yield_weights.png}
\caption{Effective emergence-time weighting of the final dipole: the
yield weight $\omega(t)$ (kernel $\times$ source) compared with the
source envelope $s_1(t)$ alone. The weighting peaks two years after cycle
maximum and is slightly negative for emergence in the first two years,
when the belt sits above the sign-change latitude of the pair yield
(Section~\ref{sec:physical_validation}).}
\label{fig:yield_weights}
\end{figure}
...
model directly (Figures~\ref{fig:app_kernel} and~\ref{fig:app_point_yield}).
The yield of a point source at latitude $\lambda$, relative to the dipole
it deposits, is approximately Gaussian with width $29.2^\circ$ at the
reference parameters (converged already within the 11-yr window). The physically relevant quantity for bipolar regions,
...
prediction exists -- the hardest test for a source-side factor
(Figure~\ref{fig:inflow_validation}). At the reference inflow amplitude
($2~{\rm m~s^{-1}}$ at $T=1$, scaling as $T^2$), the effective algebraic
strength at the calibration point is $b_{\rm eff}(T{=}1)=0.40$, with a
leading-order fit $b_{\rm IQ}\simeq0.42$ over $T\le1.2$; the algebraic
form is adequate for weak-to-moderate cycles, but $b_{\rm eff}$ grows
with $T$ (full-scan fit $1.5$; Figure~\ref{fig:app_beff}): the physical
nonlinearity is super-quadratic, and the constant-$b$ Pad\'e form
...
follows: the value $b_{\rm IQ}=0.2$ adopted from \citet{Talafha2025}
corresponds to inflow amplitudes of roughly $1~{\rm m~s^{-1}}$ at cycle
maximum in this model's convention.

\begin{figure}
\centering
\includegraphics[width=\linewidth]{figV4_inflow_validation.png}
\caption{Physical converging inflows versus the algebraic closure, in
units of the linear yield $a$: SFT scans at inflow amplitudes of $2$ and
$5~{\rm m~s^{-1}}$, with leading-order algebraic fits. The stronger
inflow reverses the dipole sign at $T=1.40$.}
\label{fig:inflow_validation}
\end{figure}
...
(Figures~\ref{fig:time_series} and~\ref{fig:bifurcation_comparison}): the
one-step prediction error falls from $97$~per~cent (memoryless) to
$1.3$\,--\,$1.5$~per~cent; the simulated attractor bifurcates to period
two exactly at the corrected analytic boundary $D_0=3.35$ (memoryless
prediction: $2.80$) and follows the corrected map through the subsequent
cascade, including the phase of the even--odd amplitude alternation and
grand-minimum-like excursions in the chaotic regime. The measurement of
the memory parameter and the displacement of the return map are shown in
Figures~\ref{fig:app_memory} and~\ref{fig:app_return_map}.

\begin{figure}
\centering
\includegraphics[width=\linewidth]{figM2_time_series.png}
\caption{Cycle-amplitude sequences of the multi-cycle SFT with dynamo
feedback compared with the memory-corrected and memoryless maps, at a
stable gain (top), just beyond the period-doubling boundary (middle), and
in the chaotic regime (bottom). In the period-two regime the
memory-corrected map tracks the simulation cycle by cycle, including the
phase of the even--odd alternation.}
...
(Figure~\ref{fig:constant_maps}) yields three regularities.

\begin{figure}
\centering
\includegraphics[width=\linewidth]{figP1_transport_constant_maps.png}
\caption{Derived transport constants of the reduced theory over the
$(\eta,u_0)$ plane at $\tau=\infty$: the pair-yield effectivity range
$\lambda_R$, the sign-change latitude $\lambda_{\rm rev}$, the cycle
memory $r$, and the linear dipole yield $a$.}
\label{fig:constant_maps}
\end{figure}
...
verified numerically at $\tau=5$~yr to within $10^{-4}$\,--\,$10^{-2}$
(Figure~\ref{fig:app_taulaws}). No
...
This appendix collects supporting figures for results quoted in the main
text. All figures, tables, and quoted numbers are regenerated by the
scripts in the repository accompanying this paper.

\begin{figure}
\centering
\includegraphics[width=\linewidth]{figF1_kernel_evolution.png}
\caption{Dipole-yield kernel $K(\mu,t)$ at different emergence times
(Section~\ref{sec:kernel_framework}). Flux emerging early in the cycle
has its yield profile fully relaxed by cycle end; late-cycle emergence
retains a sharper profile.}
\label{fig:app_kernel}
\end{figure}

\begin{figure}
\centering
\includegraphics[width=\linewidth]{figF2_intrinsic_effectivity_range.png}
\caption{Point-source dipole yield versus emergence latitude at the 11-yr
horizon and after a further 40~yr of source-free evolution, with Gaussian
fits (Section~\ref{subsec:effectivity_ranges}). The two horizons
coincide, showing that the kernel converges within the cycle window.}
\label{fig:app_point_yield}
\end{figure}

\begin{figure}
\centering
\includegraphics[width=\linewidth]{figF5_bifurcation.png}
\caption{Bifurcation diagrams of the memoryless reduced cycle map
(Section~\ref{subsec:cycle_map_stability}): attractor versus gain $D_0$
for the reference quenchings, with the analytic period-doubling boundary
marked, and versus the latitude-quenching strength at fixed $D_0=3$.}
\label{fig:app_bifurcation}
\end{figure}

\begin{figure}
\centering
\includegraphics[width=\linewidth]{figV5_binflow_effective.png}
\caption{Constancy test of the algebraic inflow closure
(Section~\ref{sec:physical_validation}): the effective strength
$b_{\rm eff}(T)=(1/Q_{\rm IQ}-1)/T^2$ for physical inflows of $2$ and
$5~{\rm m~s^{-1}}$, compared with the calibrated $b_{\rm IQ}=0.2$. The
growth of $b_{\rm eff}$ with $T$ shows the super-quadratic character of
the physical nonlinearity.}
\label{fig:app_beff}
\end{figure}

\begin{figure}
\centering
\includegraphics[width=\linewidth]{figM1_memory_parameter.png}
\caption{Measurement of the cycle memory (Section~\ref{sec:multicycle}):
source-free survival of the dipole over successive 11-yr periods,
compared with the dominant mode of the one-period propagator.}
\label{fig:app_memory}
\end{figure}

\begin{figure}
\centering
\includegraphics[width=\linewidth]{figM4_return_map.png}
\caption{Return map $(T_n,T_{n+1})$ of the multi-cycle SFT at a modulated
gain (Section~\ref{sec:multicycle}). The simulated points fall on the
memory-corrected curve $T[D_0Q-r]$, visibly displaced from the memoryless
curve $D_0TQ$.}
\label{fig:app_return_map}
\end{figure}

\begin{figure}
\centering
\includegraphics[width=\linewidth]{figP3_lambdarev_vs_lambdaR.png}
\caption{Sign-change latitude of the pair yield against the effectivity
range across the $(\eta,u_0)$ plane
(Section~\ref{sec:parameter_space}), with the proportionality of
Equation~(\ref{eq:lambda_rev_law}).}
\label{fig:app_lambdarev}
\end{figure}

\begin{figure}
\centering
\includegraphics[width=\linewidth]{figP4_tau_laws.png}
\caption{Verification of the analytic decay-time laws of
Equation~(\ref{eq:tau_laws}): the memory parameter follows
$r(\tau)={\rm e}^{-P/\tau}r(\infty)$, while the effectivity range is
invariant under $\tau$.}
\label{fig:app_taulaws}
\end{figure}
...
The code, documentation, and data that reproduce all results, tables, and
figures in this paper are available in the repository
\url{https://github.com/mtalafha90/reducedSFT}.
...
\bibitem[Biswas, Karak, and Cameron(2022)]{Biswas2022}
Biswas, A., Karak, B.B., Cameron, R.: 2022, Toroidal flux loss due to
flux emergence explains why solar cycles rise differently but decay in a
similar way. \prl\ {\bf 129}, 241102.
...
Jha, B.K., Karak, B.B., Mandal, S., Banerjee, D.: 2020, Magnetic field
dependence of bipolar magnetic region tilts on the Sun: indication of
tilt quenching. \apjl\ {\bf 889}, L19.
...
\bibitem[Karak and Miesch(2017)]{KarakMiesch2017}
Karak, B.B., Miesch, M.: 2017, Solar cycle variability induced by tilt
angle scatter in a Babcock--Leighton solar dynamo model. \apj\
{\bf 847}, 69.
...
\bibitem[Lemerle and Charbonneau(2017)]{Lemerle2017}
Lemerle, A., Charbonneau, P.: 2017, A coupled $2\times2$D
Babcock--Leighton solar dynamo model. II. Reference dynamo solutions.
\apj\ {\bf 834}, 133.
...
\bibitem[Teweldebirhan, Miesch, and Gibson(2024)]{Teweldebirhan2024}
Teweldebirhan, K., Miesch, M., Gibson, S.: 2024, Inflows towards bipolar
magnetic active regions and their nonlinear impact on a three-dimensional
Babcock--Leighton solar dynamo model. \solphys\ {\bf 299}, 42.

\bibitem[Yeates et al.(2023)]{Yeates2023}
Yeates, A.R., Cheung, M.C.M., Jiang, J., Petrovay, K., Wang, Y.-M.: 2023,
Surface flux transport on the Sun. \ssr\ {\bf 219}, 31.
